...
const EgyptGallery = () => {
    ...
    useEffect(() => {
        document.title = `${t.countries.egypt.galleryTitle} | My Story Journey`;
    }, [language, t]);
    ...
    const isFaceImage = (src) => {
        ...
        const faceKeywords = [
            'Muco', 'bro', 'friends', 'person', 'selfie', 'conference', 'minister',
            'mikel', 'salma', 'mustafa', 'hidja', 'hydja', 'bash', 'tween', 'aug', 'Augustin',
            'alexlibrary', 'bubumaad', 'scale', 'muntaza'
        ];
        return faceKeywords.some(keyword => src.toLowerCase().includes(keyword.toLowerCase()));
    };

    return (
        <div className="gallery-page theme-egypt">
            <ScrollProgress />
            <nav className="navbar">
                <div className="nav-content">
                    ...
                    <div className="nav-links">
                        <Link to="/egypt">{t.countries.egypt.backToCountry}</Link>
            ...
            <main className="container page-content">
                <section className="glass" style={{ padding: '3rem', marginTop: '2rem' }}>
                    ...
                    <div className="gallery-grid animated-grid-bg" style={{
                        ...
                        {visibleImages.length > 0 ? (
                            visibleImages.map((src, index) => {
                                // Dynamic sizing for certain indices to create a masonry look
                            ...
                            <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.5)' }}>
                                <p>No images found in the Egypt gallery folder.</p>
                            </div>
                        )}
                    </div>
                    {hasMore && (
                    ...
                    <div style={{ marginTop: '3rem' }}>
                        <Link to="/egypt" className="cta-button">{t.countries.egypt.backToCountry}</Link>
                    </div>
                </section>
            </main>
