...
const CanadaGallery = () => {
  ...
  useEffect(() => {
    document.title = `${t.countries.canada.galleryTitle} | My Story Journey`;
  }, [language, t]);
  ...
  const images = import.meta.glob("../canada_gallery/*.{jpg,jpeg,png,JPG}", {
    eager: true,
    query: "?url",
    import: "default",
  });
  ...
  return (
    <div className="gallery-page theme-canada">
      <ScrollProgress />
      ...
      <main className="container page-content">
        ...
        >
          <h1>{t.countries.canada.galleryTitle}</h1>
          <p
            style={{
              fontSize: "1.2rem",
          ...
          >
            {visibleImages.length > 0 ? (
              visibleImages.map((src, index) => {
                const isWide = [4, 9, 14, 19, 24, 29, 34, 39].includes(index + 1);
                return (
                  <div
                    key={index}
          ...
          <div style={{ marginTop: "3rem" }}>
            <Link to="/canada" className="cta-button">
              {t.countries.canada.backToCountry}
            </Link>
          </div>
        </section>
      </main>
